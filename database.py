...
def upsert_node(hid: str, node_id: str, role: str, nickname: Optional[str] = None):
    ...
    node = NodeDoc(
        ...
        sensor_reading=existing_data.get("sensorReading", 0),
        movement_pct=existing_data.get("movementPct", 0),
        warning_type=existing_data.get("warningType"),
...
def update_node_from_reading(hid: str, node: NodeReading):
    ...
    existing = db.collection("nodes").document(doc_id).get()
...
def set_node_armed(hid: str, node_id: str, armed: bool):
    """Called ONLY after explicit ESP confirmation. Never from telemetry."""
    doc_id = f"{hid}_{node_id}"
    try:
...
def set_node_requested_armed(hid: str, node_id: str, requested_armed: bool):
    """Called when the app requests arm/disarm. Only requestedArmed changes."""
    doc_id = f"{hid}_{node_id}"
    try:
...
_event_buffers: dict[str, list[CacheEntry]] = {} # eid -> pending chunk
_event_chunk_counters: dict[str, int] = {} # eid ->  next chunk index
_idle_streaks: dict[str, int] = {} # hid -> consecutive idle count
_buzzer_active: dict[str, bool] = {} # hid -> buzzer state
_known_nodes: set[str] = set() # seen "{hid}_{node_id}" keys
_pending_commands: dict[tuple, threading.Timer] = {} # (hid, node_id, cmd) → timer
...
def _update_idle_streak(hid: str, entry: CacheEntry) -> int:
    has_warning = any(n.warning_type is not None for n in entry.nodes.values())
    has_report = any(n.report_type is not None for n in entry.nodes.values())

    if entry.is_alarm or has_warning or has_report:
        _idle_streaks[hid] = 0
    else:
...
def append_to_cache(hid: str, pkg: Package) -> dict:
    ...
    entries = _in_memory_cache.setdefault(hid, [])
...
def dump_cache_to_firestore(hid: str):
    entries = _in_memory_cache.get(hid, [])
    db.collection("cache").document(hid).set({
        "packages": [e.model_dump(by_alias=True) for e in entries],
        "updatedAt": datetime.now(timezone.utc),
    })

    print(f"[DB] Cache snapshot written for {hid}: {len(entries)} packages")

def update_last_package(hid: str, entry: CacheEntry):
    db.collection("homes").document(hid).update({
        "lastPackage": entry.model_dump(by_alias=True)
    })
...
def update_event(hid: str, eid: str, entries: list[CacheEntry]):
    ...
    print(f"[DB] Event {eid}: chunk {cid} saved ({len(entries)} packages)")

def append_to_event_buffer(hid: str, eid: str, entry: CacheEntry):
    entries = _event_buffers.setdefault(eid, [])
    entries.append(entry)

    if len(entries) >= EVENT_CHUNK_SIZE:
        update_event(hid, eid, entries)
        entries.clear()

def _flush_event_buffer(hid: str, eid: str):
    entries = _event_buffers.get(eid, [])
    if entries:
        update_event(hid, eid, entries)
        entries.clear()
...
def send_buzzer_to_home(hid: str, cmd: str):
    ...
    for node in get_nodes_for_home(hid):
    ...
    print(f"[DB] Buzzer '{cmd}' sent to all nodes of home {hid}")

def _on_buzzer_timeout(hid: str, node_id: str, cmd: str):
    _pending_commands.pop((hid, node_id, cmd), None)
    print(f"[DB] Buzzer '{cmd}' to node {node_id} timed out (best-effort, no revert)")
